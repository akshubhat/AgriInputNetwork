...
exports.deploy = function(req, res, next) {
    ...
    let archiveFile = fs.readFileSync(path.join(path.dirname(require.main.filename),'agricultureinput-network/dist',req.body.myArchive));
...
/**
 * retrieve array of asset registries
 */
exports.getAssetRegistries = function(req, res, next)
{
    ...
    return businessNetworkConnection.connect(config.composer.adminCard)
        .then(() => {
            ...
            .then(function(assetRegistries){
                for (let each in assetRegistries)
                    { (function (_idx, _arr)
                        {
                        let r_type = _arr[_idx].name.split('.');
                        //console.log('asd'+r_type);
            ...
            .catch((error) => {console.log('error with getAllAssetRegistries', error);});
        })
        .catch((error) => {console.log('error with business network Connect', error);});
};
...
/**
 * gets the assets from the Asset registry
 */
                        ...
                        .then ((members) => {
                            ...
                            for (let each in members)
                                { (function (_idx, _arr)
                                    ...
                                    {
                                    case 'Retailer':
                                        if (req.body.id === _arr[_idx].retailer.$identifier)
                                        {
    ...
    .then(() => {
        ...
        .then(function(participantRegistry){
            ...
            .catch((_res) => {
                ...
                .then(() => {console.log(req.body.id +' successfully added'); res.send(req.body.id+ 'successfully added');})
                .catch((error) => {console.log(req.body.id+' add failed',error); res.send(error);});
            });
        })
